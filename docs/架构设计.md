...
该层为独立的 Ontop Endpoint Docker 容器，提供：
...
- `SparqlProxyController.java`：5 端点（query/reformulate/history/delete/status）
...
sequenceDiagram
    ...
    UI->>Engine: POST /sparql/query
    Engine->>EP: POST /sparql（form-encoded）
    EP-->>Engine: 查询结果
    Engine->>DB: 写入 query_history
    Engine-->>UI: 返回结果
    UI->>Engine: POST /sparql/reformulate
    Engine->>EP: GET /ontop/reformulate
    EP-->>Engine: SQL
    Engine-->>UI: SQL
```
